...
def merge_dicts(dict1, dict2):
    ...
    for app_name, app_data in dict2.items():
        if app_name in merged_dict:
            for test_source, test_source_data in app_data.items():
                if test_source in merged_dict[app_name]:
                    for metric, metric_data in test_source_data.items():
                        merged_dict[app_name][test_source][metric] = {
                            **merged_dict[app_name][test_source].get(metric, {}),
                            **metric_data
                        }
                else:
                    merged_dict[app_name][test_source] = test_source_data
        else:
    ...
    with open(file_path, 'r') as csv_file:
        ...
        for row in csv_reader:
            ...
            if app_name not in result.keys():
                ...
                if test_type not in result[app_name].keys():
                    result[app_name][test_type] = {
                        step: measured
                    }
                else:
                    result[app_name][test_type][step] = measured
                    
    result[app_name][test_type] = dict(sorted(result[app_name][test_type].items()))
